...
echo
oc process -f $SCRIPT_BASE_DIR/support/templates/postgresql-persistent1.json -v=POSTGRESQL_DATABASE=$POSTGRESQL_DATABASE,POSTGRESQL_USER=$POSTGRESQL_USER,POSTGRESQL_PASSWORD=$POSTGRESQL_PASSWORD  | oc create -n $OSE_CI_PROJECT -f- >/dev/null 2>&1
...
# SONRADAN EKLENENLER
oc project enterprise-resources
oc process -f "./support/templates/custom-base-image-template.json" | oc -n enterprise-resources create -f-
oc import-image base-centos -n enterprise-resources
